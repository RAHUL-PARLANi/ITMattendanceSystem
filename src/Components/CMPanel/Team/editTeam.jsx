...
const EditTeam = () => {
  ...
  const handleSubmit = () => {
    setIsLoading(true);
    axiosInstance
      .patch("/team/" + window.location.href.split("/").pop(), FormTillNow)
      .then((res) => {
        setIsLoading(false);
        toast.success(`Team Member ${res.data.name} is Edited Successfully !`);
      })
      .catch((err) => {
        console.log(err);
        setIsLoading(false);
  ...
  const handleImage = (e) => {
    ...
    fetch("https://api.cloudinary.com/v1_1/dvg9z8ugk/image/upload", {
      ...
      .then((data) => {
        console.log(data);
        setFormTillNow({ ...FormTillNow, image: data.secure_url });
      })
  ...
  useEffect(() => {
    setIsLoading(true);
    axiosInstance
      .get("/team/" + window.location.href.split("/").pop())
      .then((res) => {
      ...
      <div className="container-xxl flex-grow-1 container-p-y mt-4">
        <div className="card w-100 rounded shadow-sm col-12 mb-4">
          <div className="card-header d-flex justify-content-between align-items-center">
            <h5 className="mb-0">Create Gallery</h5>
          </div>
        ...
        <div className="" style={{ display: "flex", justifyContent: "center" }}>
          <TeamComponent data={FormTillNow} />
        </div>
      </div>
    </div>
  );
